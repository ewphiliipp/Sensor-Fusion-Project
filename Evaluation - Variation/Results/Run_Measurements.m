...
for RUN_IDX__ = 1:numel(run_dirs)
    ...
    % Get all the time information from the data. (Everything has to be in seconds)
    t_acc  = Accel.Time_s_;
    t_gyro = Gyro.Time_s_;
    t_gps  = Gps.Time_s_;
    t_mag  = Mag.Time_s_;
    ...
    % 1. FIND STATIONARY WINDOW (Robuste Version beibehalten)
    ...
    % Fallback if no stationary points
    ...
    % 1.0 = Pure Barometer | 0.0 = Pure EKF | 0.8 = Recommended (Mostly Baro, some EKF)
    alpha_alt = 0.5; 
    
    % 1. converting Barometer to elevation estimation 
    P_ref = 1013.25; 
    alt_baro = 44330 * (1 - (timetable_synced.Baro_P / P_ref).^(1/5.255));
    
    % 2. Get EKF vertical position (Z-axis)
    % Assuming EKF_Pos_z is the vertical state from your EKF
    alt_ekf = timetable_fused.EKF_Pos_z; 
    
    % 3. Weighted Fusion
    % We combine both signals before smoothing
    alt_fused_raw = (alpha_alt * alt_baro) + ((1 - alpha_alt) * alt_ekf);
    
    % 2. massive smoothing to reduce noise and reduce small differences
    alt_smooth = movmean(alt_fused_raw, 60 * fs); 
    ...
    % 5. Gesamtdistanz akkumulieren
    Distance_m_hybrid = cumsum(hybrid_inc);
    
    % 6. Ergebnisse in Tabelle speichern
